...
- (void)depthSortViews
{
    ...
    for (UIView *view in [[itemViews allValues] sortedArrayUsingFunction:compareViewDepths context:self])
    {
...
- (CGFloat)minScrollDistanceFromOffset:(CGFloat)fromOffset toOffset:(CGFloat)toOffset
{
    ...
    if (shouldWrap)
    {
		...
		return (fabs(directDistance) <= fabsf(wrappedDistance))? directDistance: wrappedDistance;
    }
    return directDistance;
}
