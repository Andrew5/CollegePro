...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(banGestureRecognizer:) name:BanGestureRecognizer object:nil];
}
...
- (void)addGestureRecognizers {
    ...
    [[self.centerViewController view] addGestureRecognizer:[self panGestureRecognizer]];//中间view左划效果,在教师工具的时候需要ban掉这个手势
}

-(void)banGestureRecognizer:(NSNotification *)notification
{
    NSDictionary *dict=notification.userInfo;
    
    if ([[dict objectForKey:@"isban"] integerValue]==1) {
        [[self.centerViewController view] removeGestureRecognizer:_panGestureRecognizer];
    }else
    {
        [[self.centerViewController view] addGestureRecognizer:_panGestureRecognizer];
    }
}
